...
impl<const A: i64, const B: i64> Add<Point<A, B>> for Point<A, B> {
    ...
    fn add(self, other: Self) -> Self {
        match (self.clone(), other.clone()) {
            (Point::Point(_,_), Point::Infinity) => self,
            (Point::Infinity, Point::Point(_,_)) => other,
            (Point::Infinity, Point::Infinity) => Point::new_infinity(),
            (Point::Point(x1,y1), Point::Point(x2,y2)) if x1 == x2 && y1 != y2 => Point::new_infinity(),
